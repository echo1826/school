...
class ListInterface {
 public:
  ...
  // Note we are not including a virtual destructor here because we aren't using
  // polymorphism. If we wanted to have a list of pointers to ListInterfaces
  // with a mix of array and linked implementations, that we could then call
  // delete on, then we would need the virtual destructor here.
};
...
void testArrayList() {
  ...
  assert(!array0.remove(1));
  assert(array0.insert(1, 10));
  assert(array0.getEntry(1) == 10);
  array0.setEntry(1, 100);
  assert(array0.getEntry(1) == 100);
  assert(array0.insert(1, 10));
  assert(array0.getEntry(1) == 10);
  assert(!array0.isEmpty());
  assert(array0.remove(1));
...
class Node {
 ...
 public:
  ...
  T getItem() const { return value; }

  Node *getNext() const { return next; }

  void setNext(Node *n) { next = n; }

  void setItem(const T &v) { value = v; }
};
  ...
  void setEntry(int position, const ItemType &newEntry) {
    if (position < 1 || position > itemCount) {
      throw(std::invalid_argument(
          "setEntry() called with an empty list or invalid position"));
    }
...
void testLinkedList() {
  ...
  assert(!list0.remove(1));
  assert(list0.insert(1, 10));
  assert(list0.getEntry(1) == 10);
  list0.setEntry(1, 100);
  assert(list0.getEntry(1) == 100);
  assert(list0.insert(1, 10));
  assert(list0.getEntry(1) == 10);
  assert(!list0.isEmpty());
  assert(list0.remove(1));
...
void insertionSort(ListInterface<ItemType> &list) {
  ...
  for (int i = 2; i <= list.getLength(); i++) {
    int currentValue = list.getEntry(i);
    int j = i - 1;
    while (j >= 1 && list.getEntry(j) > currentValue) {
      list.setEntry(j + 1, list.getEntry(j));
      j = j - 1;
    }
    list.setEntry(j + 1, currentValue);
  }
}
